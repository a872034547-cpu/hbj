...
def write_markdown_file(file_path: str, content: str) -> None:
    ...
    写入章节正文属于不可静默失败的关键路径。旧实现捕获异常后只打印日志，
    会让上层误以为章节已写入，最终在 Windows 非法路径等场景中只看到
    ``[Errno 22] Invalid argument`` 的笼统提示。这里保留日志，但必须继续
    抛出异常，由 UI/任务层展示可读错误并阻止错误状态落盘。
    """
    try:
        Path(file_path).parent.mkdir(parents=True, exist_ok=True)
        ...
        raise OSError(f"无法写入 Markdown 文件：{file_path}。原始错误：{e}") from e
